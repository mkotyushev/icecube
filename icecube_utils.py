...
def angular_error_task_direction(preds: List[Tensor], data: Data) -> Tensor:
    """
    Calculates angular error for direction task.

    Args:
        preds: Tasks predictions len(preds) == 1, 
            preds[0] -- direction task predictions, 
            preds[0].shape (N, 4) -- (x, y, z, kappa)
        data: Batch data
...
def angular_error_task_s2(preds: List[Tensor], data: Data) -> Tensor:
    """
    Calculates angular error for s2 task.

    Args:
        preds: Tasks predictions len(preds) == 2, 
            preds[0] -- s2 abs task predictions, 
            preds[0].shape (N, 3) -- (x, y, z)
            preds[0] in [0, 1]

            preds[1] -- s2 sign task predictions,
    ...
    pred_z_abs = preds[0][:, 2].cpu().numpy()
...
def build_model(
    ...
    if config["target"] == 'direction':
        ...
        additional_attributes = ['zenith', 'azimuth', 'event_id']
        metric_fn = angular_error_task_s2
